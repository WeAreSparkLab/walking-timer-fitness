...
import { sendWebPushNotification } from '../lib/webNotifications';
...
export default function Dashboard() {
  ...
  const handleShareToFriend = async (friendId: string, friendName: string, link: string) => {
    try {
      ...
      // 1. Create in-app notification (works everywhere)
      await createNotification(
        friendId,
        'walk_invite',
        '🚶 Walk Invitation',
        `${senderName} invited you to join "${walkName}"`,
        {
          inviteLink: link,
          sessionId: selectedSession.id,
          sessionName: walkName,
        }
      );
      
      // 2. Send web push notification (for browser users)
      try {
        if (Platform.OS === 'web') {
        ...
        console.log('Web push not sent:', webPushError);
      }
      
      // 3. Send mobile push notification (for app users)
      try {
        await sendWalkInviteToFriend(friendId, walkName, link);
      } catch (pushError) {
        console.log('Mobile push not sent:', pushError);
      }
      
      // 4. Copy link as backup
      if (Platform.OS === 'web') {
        await navigator.clipboard.writeText(link);
      }
