  ...
  const handleShareToFriend = async (friendId: string, friendName: string, link: string) => {
    try {
      ...
      if (notificationSent) {
        if (Platform.OS === 'web') {
          window.alert(
            `✅ Notification sent to ${friendName}!\n\nThey'll receive a notification about your walk invitation.\n\nLink also copied to clipboard as backup.`
          );
        } else {
          Alert.alert(
            '✅ Invitation Sent!',
            `${friendName} will receive a notification about your walk invitation.`,
            [{ text: 'OK' }]
          );
        }
      } else {
        ...
        if (Platform.OS === 'web') {
          window.alert(
            `Link copied to clipboard!\n\nNotification failed to send, but you can paste and share the link with ${friendName} manually.`
          );
        } else {
          Alert.alert(
            'Link Copied',
            `Share this link with ${friendName}.`,
            [{ text: 'OK' }]
          );
        }
      }
